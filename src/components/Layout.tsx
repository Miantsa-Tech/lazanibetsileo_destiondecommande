...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] dark:bg-slate-900 flex">
      {/* Overlay mobile */}
      {sidebarOpen && (
      ...
      <aside className={`
        ...
        w-64 bg-[#2D5016] dark:bg-slate-800 text-white
      ...
      `}>
                ...
                <NavLink
                  ...
                  className={({ isActive }) => `
                    ...
                    ${isActive
                      ...
                      : 'text-green-100 dark:text-slate-300 hover:bg-white/10 dark:hover:bg-slate-700 hover:text-white'
                    }
                  `}
                >
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3">
            {user?.photo ? (
              <img src={user.photo} alt="Profil" className="w-9 h-9 rounded-full object-cover" />
            ) : (
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-green-300 dark:text-slate-400 capitalize">{user?.role}</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-4 lg:px-6 py-3 flex items-center justify-between sticky top-0 z-30 shadow-sm">
          <div className="flex items-center gap-3">
            <button
            ...
            <div className="relative">
              ...
              >
                {user?.photo ? (
                  ...
                  <div className="w-8 h-8 bg-[#2D5016] dark:bg-emerald-700 rounded-full flex items-center justify-center text-white text-xs font-bold">
                    {user?.prenom?.[0]}{user?.nom?.[0]}
                  </div>
                )}
              ...
              {userMenuOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-gray-200 dark:border-slate-700 py-1 z-50">
                  <div className="px-4 py-2 border-b border-gray-100 dark:border-slate-700">
                    <p className="text-sm font-medium text-gray-800 dark:text-slate-100">{user?.prenom} {user?.nom}</p>
                    <p className="text-xs text-gray-500 dark:text-slate-400">{user?.email}</p>
                  </div>
                  <button
                    onClick={() => { setUserMenuOpen(false); setProfileModalOpen(true); }}
                    className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center gap-2"
                  >
